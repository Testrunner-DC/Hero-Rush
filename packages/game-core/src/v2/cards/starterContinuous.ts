...
export function starterContinuousDeltaV2(
  ...
  for (const [hostCardId, attachedIds] of Object.entries(state.attachments)) {
    ...
    for (const attachedId of attachedIds) {
      const cardNo = state.cards[attachedId]?.cardNo;
      if (cardNo === "SD01-002") delta += type === "range" ? 2 : type === "power" ? 2500 : 0;
      if (cardNo === "SD01-010" && type === "range") delta += 1;
      if (cardNo === "SD01-016" && type === "power") delta += 1000;
      if (cardNo === "SD04-001" && type === "power") delta += 1000;
    }
  }

  if (target.cardNo === "SD01-004" && type === "power" && battleZoneCard(state, owner, targetCardId) && state.activePlayer === opponent && isBattlePhase(state)) {
    delta += fieldZones.reduce((count, zone) => count + state.players[opponent].field[zone].length, 0) * 1000;
  }
  if (target.cardNo === "SD01-008" && type === "level" && state.players[owner].hand.includes(targetCardId)) {
  ...
  if (target.cardNo === "SD02-001" && type === "power" && battleZoneCard(state, owner, targetCardId)) delta += state.players[owner].baseCovered.length * 1000;
  if (target.cardNo === "SD02-002" && type === "range" && battleZoneCard(state, owner, targetCardId)) delta += state.players[owner].baseCovered.length;

  for (const source of Object.values(state.cards).filter((card) => card.owner === owner && faceUpOnField(state, owner, card.instanceId))) {
    if (source.cardNo !== "SD02-001" || type !== "power" || !battleZoneCard(state, owner, source.instanceId)) continue;
  ...
  for (const source of Object.values(state.cards).filter((card) => card.owner === opponent && faceUpOnField(state, opponent, card.instanceId))) {
    if (source.cardNo === "SD02-003" && type === "power" && fieldZoneOf(state, opponent, source.instanceId) === "rear" && fieldZoneOf(state, owner, targetCardId) === "vanguard") delta -= 500;
  }
  if (target.cardNo === "SD02-018" && type === "power" && battleZoneCard(state, owner, targetCardId)) {
    const enemyVanguard = state.players[opponent].field.vanguard[0];
    if (enemyVanguard && resolve(enemyVanguard, "range") === 1) delta += 1500;
  }
  if (target.cardNo === "SD03-004") {
  ...
  for (const source of Object.values(state.cards).filter((card) => card.owner === opponent && faceUpOnField(state, opponent, card.instanceId))) {
    ...
    if (source.cardNo === "SD03-011" && type === "power") {
    ...
    if (source.cardNo === "SD04-016" && type === "level" && targetZone === "rear") delta -= 2;
  }
  return delta;
}
...
export function starterContinuousReplacementV2(
  ...
  for (const [hostCardId, attachedIds] of Object.entries(state.attachments)) {
    if (hostCardId !== targetCardId) continue;
    if (attachedIds.some((id) => state.cards[id]?.cardNo === "SD04-001")) return 0;
    if (attachedIds.some((id) => state.cards[id]?.cardNo === "SD03-016")) return 1;
  }
  return null;
}
...
export function starterContinuousKeywordsV2(state: GameStateV2, cardId: string): OfficialKeywordV2[] {
  ...
  if (!card || card.cardNo !== "SD01-017" || !battleZoneCard(state, card.owner, cardId)) return [];
...
export function preventsTieRetreatV2(state: GameStateV2, cardId: string): boolean {
  const card = state.cards[cardId];
  return Boolean(card && card.cardNo === "SD02-003" && fieldZoneOf(state, card.owner, cardId) === "vanguard");
}
