...
const definitions: EffectDefinitionV2[] = [
  ...
  },
  {
    ...
    condition: (state, actor, _source, context) => { const event = eventOf(context, "CARDS_RETREATED")!; return (event.fromFieldCardIds ?? event.cardIds).some((id) => { const zone = event.fromFieldZones?.[id]; return Boolean(zone && state.players[actor].retreat.includes(id) && state.players[actor].field[zone].length === 0 && state.cards[id]?.owner === actor && effectiveValueV2(state, id, "level") <= 4 && state.cards[id]?.features.some((feature) => feature.includes("人类"))); }); },
    buildOperations: (state, actor, source, _selected, context) => { const event = eventOf(context, "CARDS_RETREATED")!; const target = (event.fromFieldCardIds ?? event.cardIds).find((id) => { const zone = event.fromFieldZones?.[id]; return Boolean(zone && state.players[actor].retreat.includes(id) && state.players[actor].field[zone].length === 0 && state.cards[id]?.owner === actor && effectiveValueV2(state, id, "level") <= 4 && state.cards[id]?.features.some((feature) => feature.includes("人类"))); })!; const zone = event.fromFieldZones![target]!; return [{ kind: "BANISH", cardIds: [source], sourceCardId: source }, { kind: "PLACE_FIELD", cardId: target, destination: zone, sourceCardId: source, requiresPreviousSuccess: true }]; },
  },
  {
