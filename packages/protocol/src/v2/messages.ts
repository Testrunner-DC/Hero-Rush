...
export const AtomicOperationV2Schema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("DRAW"), actor: PlayerIndexSchemaV2, count: z.number().int().min(1).max(20), sourceCardId: IdSchemaV2.optional() }),
  ...
  z.object({ kind: z.literal("RETREAT"), cardIds: z.array(IdSchemaV2).min(1).max(40) }),
  z.object({ kind: z.literal("BANISH"), cardIds: z.array(IdSchemaV2).min(1).max(40) }),
  z.object({ kind: z.literal("MOVE_TO_BASE"), cardId: IdSchemaV2, face: z.enum(["up", "down"]) }),
  z.object({ kind: z.literal("PLACE_FIELD"), cardId: IdSchemaV2, destination: FieldZoneSchemaV2 }),
  z.object({ kind: z.literal("COVER"), cardId: IdSchemaV2 }),
  z.object({ kind: z.literal("REVEAL"), cardIds: z.array(IdSchemaV2).min(1).max(40) }),
  z.object({ kind: z.literal("FLIP_BASE_FACE_UP"), cardId: IdSchemaV2 }),
  z.object({ kind: z.literal("MOVE_TO_DECK_BOTTOM"), cardId: IdSchemaV2 }),
  z.object({ kind: z.literal("MOVE_FIELD"), cardId: IdSchemaV2, destination: FieldZoneSchemaV2 }),
  ...
  z.object({ kind: z.literal("SWAP_POSITIONS"), cardIds: z.tuple([IdSchemaV2, IdSchemaV2]) }),
  ...
  z.object({ kind: z.literal("ATTACH"), cardId: IdSchemaV2, hostCardId: IdSchemaV2 }),
  z.object({ kind: z.literal("DETACH"), cardId: IdSchemaV2, destination: z.enum(["hand", "retreat", "base"]) }),
