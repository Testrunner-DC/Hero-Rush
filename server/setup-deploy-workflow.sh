...
readonly SERVICE_USER="hero-rush"
...
if ! id -u "$SERVICE_USER" >/dev/null 2>&1; then
  useradd --system --gid "$SERVICE_GROUP" --home-dir "$DEPLOY_ROOT" --no-create-home --shell /usr/sbin/nologin "$SERVICE_USER"
fi
if ! id -u "$DEPLOY_USER" >/dev/null 2>&1; then
  useradd --create-home --shell /bin/bash "$DEPLOY_USER"
fi
usermod -a -G "$SERVICE_GROUP" "$DEPLOY_USER"

install -d -o "$DEPLOY_USER" -g "$SERVICE_GROUP" -m 2770 "$DEPLOY_ROOT" "$RELEASES_DIR"
install -d -o "$DEPLOY_USER" -g "$SERVICE_GROUP" -m 0750 "$SHARED_DIR"
install -o "$DEPLOY_USER" -g "$SERVICE_GROUP" -m 0640 "${LEGACY_REPO}/.env" "${SHARED_DIR}/.env"
chgrp "$SERVICE_GROUP" "${LEGACY_REPO}/.env"
...
chown -h "$DEPLOY_USER:$SERVICE_GROUP" "$CURRENT_LINK"
